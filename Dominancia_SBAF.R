...
library(ggplot2)
# Paso 1: Leer base
SBAF_ACCESOS_MUNICIPIO <- read_excel("~/SBAF_ACCESOS_MUNICIPIO_IHH.xlsx")

# Paso 2: Limpiar columnas de texto
SBAF_ACCESOS_MUNICIPIO <- SBAF_ACCESOS_MUNICIPIO %>%
  mutate(across(where(is.character), ~iconv(., from = "", to = "UTF-8", sub = " ")))
# Paso 3: Conteo de municipios en situación de dominancia
resumen_dominancia <- SBAF_ACCESOS_MUNICIPIO %>%
  count(dominancia) %>%
...
ggplot(resumen_dominancia, aes(x = "", y = porcentaje, fill = dominancia)) +
  ...
  coord_polar(theta = "y") +
  labs(title = "Distribución de Dominancia en Municipios (SBAF - Sep 2023)",
...
dominancia_por_empresa <- SBAF_ACCESOS_MUNICIPIO %>%
